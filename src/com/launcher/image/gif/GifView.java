package com.launcher.image.gif;
...
{
	/** 是否强制设置没帧切换的时间 */
	private Boolean forceDecodeTime = false;
	/** 强制每帧切换的时间 */
	private int time = 0;
	...
	public void stopShow()
	{
		try
		{
			Log.i( "DVB_LIVE", "interrupt" );
			dt.interrupt();
			Log.i( "DVB_LIVE", "interrupt end" );
			dt = null;
			Log.i( "DVB_LIVE", "interrupt null" );
		}
		catch ( Exception e )
		{
			Log.i( "DVB_LIVE", "interrupt error" + e );
		}
		try
		{
			Log.i( "DVB_LIVE", "stop" );
			gifDecoder.free();
			Log.i( "DVB_LIVE", "stop end" );
			gifDecoder = null;
			Log.i( "DVB_LIVE", "stop end null" );
		}
		catch ( Exception e )
		{
			Log.i( "DVB_LIVE", "stop error" + e );
		}
	}
	...
	private class DrawThread extends Thread
	{
		public void run()
		{
			try
			{
				...
				while ( isRun )
				{
					if ( pause == false )
					{
						...
						Log.i( "DVB_LIVE_LOG", "DrawThread gifDecoder " + gifDecoder );
						if ( gifDecoder == null )
						{
							Log.i( "DVB_LIVE_LOG", "DrawThread gifDecoder RETURN " );
							return;
						}
						GifFrame frame = gifDecoder.next();
						currentImage = frame.image;
						Log.i( "DVB_LIVE_LOG", "DrawThread currentImage " + currentImage );
						sp = frame.delay;
			...
			catch ( Exception e )
			{
				...
				Log.i( "DVB_LIVE_LOG", "DrawThread Exception " + e );
			}
		}
	}

	public void forceDecodeTime( boolean flag )
	{
		this.forceDecodeTime = flag;
	}

	public void setDecodeDelay( int time )
	{
		this.time = time;
	}
	...
	{
		return sp * gifDecoder.getFrameCount();
	}
